...
class ConditionalDDPM(nn.Module):
    ...
    def sample(self, n_sample, y, size):
        ...
        for t in reversed(range(self.T)):
            ...
            # 使用噪声预测模型预测噪声eps，输入包括当前图像x_t、条件y和时间步t
...
# 定义条件扩散模型的训练函数
def train_conditional_diffusion(diffusion, device, n_epoch=50, sample_dir='log/samples_conditional'):
...
if __name__ == "__main__":
    ...
    # 开始训练条件扩散模型
    train_conditional_diffusion(diffusion=c_ddpm, device=device)
